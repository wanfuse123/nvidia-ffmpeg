...
basedir=/tmp/ffmpeg-nvidia-012342349
ffmpegdir="$basedir/ffmpeg"
mkdir "$basedir" || printf "can not create %s \n " "$basedir"
touch /tmp/CATCH-12342342379  || printf "/tmp/CATCH-12342342379 exists continuing... \n"
rm -rf "${basedir:-/tmp/CATCH-1231231279/}" || printf "can not delete basedir: %s\n" "$basedir" 

mkdir -p "$basedir"
pushd "$basedir" || printf "can not change to basedir: %s\n" "$basedir" 
...
printf "Configured gnome to have 9999999 scrollback lines"
echo -ne '\e]11;#aaaaaa\a'

}

# End configure_Gnome_Terminal
...
pushd "$basedir" || printf 'can not change to basedir \n '

git clone --depth=1 https://github.com/OpenVisualCloud/SVT-AV1
pushd "$basedir"/SVT-AV1/Build  || printf "unable to change to SVT-AV1 build dir, please check..\n"
...
pushd "nv-codec-headers" || printf "can not change to codec-headers exiting "
make -j "$(nproc)*2"
sudo make install

git config --global --add safe.directory "$ffmpegdir"
git clone https://git.ffmpeg.org/ffmpeg.git "$ffmpegdir"



pushd "$ffmpegdir" || printf 'can not change to ffmpeg build dir \n '
printf 'selecting the remotes/origin/release/5.1 branch of ffmpeg/n'
...
touch /tmp/CATCH-12342342379  || printf "/tmp/CATCH-12342342379 exists continuing..."
#rm  "${ffmpegdir/config_components.h:-/tmp/CATCH-1231231279/}" || printf "can not delete config_components.h file: %s\config_components.h \n" "$basedir" 

cat > $ffmpegdir/config_components.h << 'FFMPEG_CONFIG_COMPENTSh_EOF'
...
# --enable-cuda-sdk
#--extra-ldflags="-L$ffmpegdir/lib -L/usr/local/cuda/lib64" \ 
#--extra-libs="-lpthread -lm" \ 
#--bindir="$HOME/bin" \ 
# --enable-cuda-nvcc --enable-cuda-nvcc
# --pkg-config-flags="--static"
# these option settings are not open source and can not be distributed as such
#--enable-libfdk-aac non-free can not use
# --enable-nonfree   non-free can not use
# --enable-libnpp  non-free can not use
# --enable-cuda  ?? doesnt exist ??
#    --enable-cuda
PATH="$HOME/bin:$PATH" 
make -j "$(nproc)*2"
make install 
hash -r
curl -F file=@"$ffmpegdir/ffbuild/./config.log" https://0x0.st


printf 'type something like : ffmpeg -y -vsync 0 -hwaccel cuda -hwaccel_output_format cuda -i ./Costa_Rica_2.mp4   -c:a copy -pix_fmt yuv420p -c:v h264_nvenc -b:v 5M Costa_Rica_2_h264_2560x1440-2.mp4  \n'

printf ' Need to add the following two lines to ~/.bashrc'
printf 'export PATH=/usr/local/cuda-11.5/bin${PATH:+:${PATH}}'
printf 'export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/usr/local/cuda/lib64'
printf 'some statistics top, htop , nvtop'
printf 'grep -i clang $ffmpegdir/ffbuild/./config to see if clang is enabled'
printf "nvcc %sversion to see if cuda is enabled" "--"
printf ' this is dependant upon apt install nvidia-driver-470'
printf 'good commands but uses propriatry packages non-free in the setup, same commands should work with this https://docs.nvidia.com/video-technologies/video-codec-sdk/ffmpeg-with-nvidia-gpu/'
